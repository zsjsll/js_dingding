...
(function main() {
	...
	function listenMsg(n: org.autojs.autojs.core.notification.Notification) {
		...
		if (n.getText() === "邮件提醒: 你有一封新邮件")
			cfg.thread = threads.start(() => phone.turnOn());
		...
		if (_.includes(n.getText(), "暂停")) {
			...
			const pause_tatus_msg = _.isEmpty(tools.pauseStatus(cfg.pause))
				? ["暂停0次, 恢复定时打卡"]
				: tools.pauseStatus(cfg.pause);
